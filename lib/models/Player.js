...
const ACTION_TYPE = require('../types/action-type');
...
function getState(playerInternal, coordinatorInternal) {
  ...
  return Object.freeze({
    ...
    allowedActions: coordinatorInternal.allowedActions,
...
function Player(data, listener, system) {
  ...
  function notificationHandler(uuid, data) {
    ...
    if (data.currenttransportactions) {
      state.allowedActions = data.currenttransportactions.val.split(', ').map(action => ACTION_TYPE[action] ? ACTION_TYPE[action] : action);
    }
